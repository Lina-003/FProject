...
class Region extends HTMLElement {
  ...
  render() {
    if (this.shadowRoot) {
      ...
      mainAmazonas.appendChild(createRegionElement(
      ...
      mainAndina.appendChild(createRegionElement(
        "Región Andina", "Con cerca de 30 millones de habitantes, es la región más poblada y de mayor desarrollo económico del país, donde se ubican los 3 centros urbanos más importantes: Bogotá, Medellín y Cali. Allí se encuentran la cultura paisa, la santandereana, la cundiboyacense, la opita, la vallecaucana y la pastusa, que además está en constante contacto con la región del Pacífico colombiano.", 
      this.home));
      this.shadowRoot?.appendChild(mainAndina);
      
      const mainCaribe = this.ownerDocument.createElement("main");
      mainCaribe.appendChild(createRegionElement(
        "Región Caribe", "Históricamente ha sido una de las regiones de Colombia más conectadas con el resto del mundo gracias a los puertos de Barranquilla y Cartagena, que a su vez, son sus dos principales centros urbanos. En esta región surgieron expresiones culturales icónicas como la cumbia, el vallenato, las artesanías del pueblo Zenú y el mundialmente famoso Carnaval de Barranquilla.", 
      this.home));
      this.shadowRoot?.appendChild(mainCaribe);

      const mainInsular = this.ownerDocument.createElement("main");
      mainInsular.appendChild(createRegionElement(
        "Región Insular", "Agrupa diferentes islas importantes alejadas del territorio continental colombiano. Esto incluye al de San Bernardo y el de San Andrés, Providencia y Santa Catalina (en el mar Caribe) y las islas Gorgona y Malpelo (en el océano Pacífico). Cuenta con una belleza marina destacable y paisajes paradisíacos con playas de arenas blancas, importantes reservas y santuarios naturales.", 
      this.home));
      this.shadowRoot?.appendChild(mainInsular);

      const mainOri = this.ownerDocument.createElement("main");
      mainOri.appendChild(createRegionElement(
        "Región Orinoquía", "Con amplias planicies que parecen infinitas, múltiples ríos, ecosistemas únicos y baja densidad poblacional, la región de la Orinoquía hace parte de la gran cuenca del río Orinoco que comparten Colombia y Venezuela.", 
      this.home));
      this.shadowRoot?.appendChild(mainOri);

      const mainPacif = this.ownerDocument.createElement("main");
      mainPacif.appendChild(createRegionElement(
        "Región Pacífica", "El Pacífico colombiano atraviesa 4 departamentos y está compuesto por largas costas y selvas que conforman uno de los territorios más agrestes y húmedos del continente, donde se concentra gran parte de la biodiversidad del país.", 
      this.home));
